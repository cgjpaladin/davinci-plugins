...
function _buildRow(f,i){
  ...
  tdNum.appendChild(Object.assign(document.createElement('span'),{textContent:i+1}));tr.appendChild(tdNum);
...
function activateShotEdit(td,i,oldVal){
  ...
  function _mkRow(v){const r=document.createElement('div');r.className='shot-edit-row';const ip=document.createElement('input');ip.value=v||'';ip.setAttribute('inputmode','numeric');
    ...
    // 失焦补零
    ip.addEventListener('blur',()=>{const x=ip.value.replace(/[^\d]/g,'');if(x&&x.length<2){ip.value=x.padStart(2,'0')}});
  ...
  const commit=(cancel)=>{
    ...
    const vals=[...ct.querySelectorAll('input')].map(ip=>{let v=ip.value.replace(/[^\d]/g,'');if(v.length>2)v=v.slice(0,2);return v?v.padStart(2,'0'):''}).filter(v=>v);
    const nv=vals.join('-');
