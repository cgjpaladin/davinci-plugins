---
name: davinci-script
description: 达芬奇 Resolve Python 脚本开发。触发词：达芬奇插件、Resolve API、时间线操作、渲染。路由器 skill——共享基础+按场景加载子 skill。
...
| API 调用没报错但没效果 | 达芬奇 API **静默失败**，必须检查返回值 | `插件排错诊断` |
...
| 远程测试 | dry-run 模式，0 积分，不调 API | `插件排错诊断` |
...
| `InsertClip`/`InsertClipIntoTrack`/`InsertClipsToTimeline` | ✅ 达芬奇 20.3.2 实测已移除。替代方案：`MediaPool.AppendToTimeline([{clipInfo}])` 配合 `recordFrame` 参数指定位置 |

**读操作不受限**：`GetNodeGraph()` / `GetNumNodes()` / `GetToolsInNode()` / `GetLUT()` 均可读。

### API 真相源

- **唯一可信源**：官方 README（`/Library/Application Support/.../Developer/Scripting/README.txt`）+ 达芬奇实测
- **不可信**：AI 生成的 API 文档/报告。`hasattr()` 和 `dir()` 会误报空壳 API（如 `GetSubtitleText` 等实际值为 None）——必须实际调用验证。
- 未公开的 `GetProperty()` 属性已实测记录在 `达芬奇API参考` skill。
...
- **外部知识入口速查（必读）**：`docs/外部知识库.md` — API 供应商 URL（无痕/鬼手/OSS）、达芬奇社区论坛、GitHub 开源项目列表。当需要查外部服务文档、找达芬奇社区资源、了解有哪些开源项目可参考时，先读这份。
...
# ❌ 直接 sed -i 批量替换——不可逆
# ✅ 先 dry-run 预览
sed -n 's/旧内容/新内容/p' 文件 | head -20   # 肉眼验证后确认
```

Python 正则中 Unicode 码点用 `chr()` 不用 `\u`——Python `\u` 只认 4 位 hex，U+2F800 等 5 位会被截断。
...
新插件（AI换口型/语音克隆等）直接继承：

- **进度条**：`BasePipeline` 含 `MILESTONE_*` 常量，`DaVinciPipelineUI.set_phase()` 直写标签，子类覆盖 `_get_progress_callback()`
- **估值常量**：`pricing_defaults.EST_BASE_SECS / EST_PER_CLIP_SECS`（基于无痕+鬼手），新工具可覆盖
python3 ~/Desktop/test_ui_progress.py
```
