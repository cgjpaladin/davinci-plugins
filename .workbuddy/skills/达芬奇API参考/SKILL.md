...
# GetProperty() 未公开属性（实测验证 2026-07-02，DaVinci 20.3.2）
# 这些属性不在官方 README 里，通过 GetProperty() 可读
props = it.GetProperty()

props.get("DynamicZoomEase", 0)   # 0=关闭, 1=线性, 2=缓入, 3=缓出, 4=缓入缓出
props.get("ZoomX", 1.0)           # X 缩放倍数
props.get("ZoomY", 1.0)           # Y 缩放倍数
props.get("ZoomGang", True)       # 等比缩放
props.get("CropLeft", 0.0)        # 左裁剪
props.get("CropRight", 0.0)       # 右裁剪
props.get("CropTop", 0.0)         # 上裁剪
props.get("CropBottom", 0.0)      # 下裁剪
props.get("RotationAngle", 0.0)   # 旋转角度
props.get("Opacity", 100.0)       # 不透明度 (0-100)
props.get("Pan", 0.0)             # 平移
props.get("Tilt", 0.0)            # 倾斜
props.get("RetimeProcess", 0)     # 变速模式 (3=光流)
props.get("ResizeFilter", 0)      # 调整大小滤镜
props.get("Scaling", 0)           # 缩放模式
props.get("MotionEstimation", 0)  # 运动估计
props.get("FlipX", False)         # 水平翻转
props.get("FlipY", False)         # 垂直翻转
...
if ng:
    for i in range(1, ng.GetNumNodes() + 1):
        ni = ng.GetNodeGraphNode(i)
        if ni:
            tools = ni.GetToolsInNode()  # 返回 OFX 工具名列表
```

**达芬奇 7 种模糊 OFX 名**（可用于调色标记检测，实测确认 20.3.2）：
...
检测逻辑：节点数 ≤2 且含以上任一种 → 可判定为打码/遮挡处理。
...
| `DynamicZoomEase` 始终为 0 | 明明开了动态缩放却检测不到 | 0 = 关闭 **或** 开了但从未调过缓动 → 值无法区分。检测只能确认 >0 时一定开了，=0 时不能断定没开 |
