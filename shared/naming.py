...
def _build_filename_re():
    """从 FIELD_CONFIG 生成解析正则——改字段顺序自动跟随，无需手动同步"""
    segments = []
    # 统计 desc 之后、v 之前的可变字段数（用于 desc 前瞻断言）
    desc_seen = False
    desc_trail = 0  # desc 和 v 之间有几个字段
    for fd in FIELD_CONFIG:
...
def build_folder(root, entry):
    """构造归档路径: root/EP/EP_SC_method_ver/filename"""
    f = entry.fields
