...
win = disp:AddWindow({
    ...
    ui:VGroup{
        ...
        ui:HGroup{
            Weight = 0.2,
            ui:Label{ Text = "匹配文件名前" ,Weight = 0.2},
            ui:SpinBox{ ID = "MatchLength", Minimum = 1, Maximum = 100, Value = 3 ,Weight = 0.15},  -- 输入匹配的n位
            ui:Label{ Text = "位" ,Weight = 0.1},
            --ui:CheckBox{ ID = "FullMatchCheckBox", Text = "包含子文件夹", Checked = false, Weight = 0.3 },
        },
        ui:HGroup{
            Weight = 0.2,
...
--[[
function win.On.FullMatchCheckBox.Clicked(ev)
    if win:GetItems().FullMatchCheckBox.Checked then
        win:GetItems().MatchLength.Enabled = false
    else
        win:GetItems().MatchLength.Enabled = true
    end
end
]]
-- 打印日志到TextEdit控件
...
-- 获取替换文件夹中的文件
function getFilesInFolder(folderPath, matchLength, fullMatch)
    ...
    for line in pfile:lines() do
        ...
        if fullMatch then
            ...
            filename = filepath:match("([^/\\]+)$")  -- 提取文件名
        else
            filepath = folderPath .. "/" .. line  -- 这里修改了代码，确保路径正确
            filename = line
        end
        ...
        if nameWithoutExtension then
            local matchKey
            if fullMatch then
                matchKey = nameWithoutExtension
            else
                matchKey = nameWithoutExtension:sub(1, matchLength)
            end
            local nextChar = nameWithoutExtension:sub(matchLength + 1, matchLength + 1)
            if fullMatch or nextChar == "" or not nextChar:match("%a") then
                t[matchKey] = filepath  -- 使用完整路径
            end
        end
    end
    pfile:close()
...
-- 获取媒体池中的素材列表
function getClipsFromMediaPool(fullMatch)
    ...
    if fullMatch then
        ...
        return current_folder:GetClipList()
    end
end
...
-- 处理素材的通用函数
function processClips(clips, a_files, matchLength, fullMatch, processClipFunc)
...
function checkFiles()
    ...
    local matchLength = win:GetItems().MatchLength.Value
    --local fullMatch = win:GetItems().FullMatchCheckBox.Checked
    ...
    -- 获取替换文件夹中的文件
    local a_files = getFilesInFolder(folderPath, matchLength, fullMatch)
    ...
    -- 获取媒体池中的素材
    local clips = getClipsFromMediaPool(fullMatch)
    ...
    -- 处理素材
    processClips(clips, a_files, matchLength, fullMatch, checkClip)
    ...
    if fullMatch then
        ...
        logMessage("匹配文件名前 " .. matchLength .. " 位:")
    end
...
function startReplace()
    ...
    local totalClips = 0  -- 可替换的素材数量，将在下面统计
    ...
    -- 遍历素材，统计可替换的素材
    processClips(clips, a_files, matchLength, fullMatch, countReplaceableClips)
    ...
    function replaceClip(clipData)
        ...
        local original_clip_name = clip:GetName()  -- 获取原始文件名
